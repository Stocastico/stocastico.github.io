...
- **Runtime palette picker**: `generate-theme` emits, besides the active palette's `:root` block, a `@theme-generated-alt` block with every *other* palette scoped to `:root[data-palette="<id>"]` (plus its `[data-theme="light"]` pair), and exports `PALETTES` + `ACTIVE_PALETTE` from `js/theme.js`. The active palette is deliberately **not** re-emitted — it already is the `:root` default, so "select the active palette" means "remove the attribute". `applyPalette(id)` (js/ui.js) pins `data-palette`, persists to `localStorage['palette']`, repoints `<meta theme-color>` and fires the same `themechange` event the light/dark toggle uses, so the colour-baked canvases rebuild. The picker lives in the ⌘K command palette under "Appearance". The `<head>` bootstrap re-applies the stored id before first paint (pattern-checked, since it has no access to the palette table — an unknown id matches no rule and falls back to the default). Costs ~2.6 KB gzip in total. **Known limitation**: the favicons and the OG card are committed per-palette static files, so they stay on the *active* palette regardless of the visitor's choice; the nav-logo gradient does follow it (its stops are `var(--accent)` / `var(--accent2)`, not baked hex).
