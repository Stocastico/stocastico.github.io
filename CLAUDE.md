...
- **CNN hero pipeline**: `scripts/train-cnn.mjs` trains a small LeNet-5 (28×28 → conv 6@5×5 → pool → conv 16@5×5 → pool → 120 → 84 → 10; ~44 k params, 98.5 % test accuracy) on MNIST in **plain JavaScript with no dependencies**, writing `data/cnn-model.json` + `data/cnn-samples.json`. `scripts/generate-cnn-activations.mjs` then replays ten forward passes, quantises every layer's activations to uint8 against a shared per-layer max and base64-packs them into `data/cnn-activations.js` (~42 KB source, ~11 KB gzip). `js/cnn-hero.js` is a *player* for that data — it draws the layers in oblique projection and animates the signal travelling through them — so **the browser runs no inference and downloads no ML runtime**. Depth comes from three things. The **static recession** (`DEPTH_SHRINK`/`DEPTH_RISE`, `_view()`/`_project()`) is the one that survives a still screenshot: the pipeline runs *away* from the viewer, so each successive layer is drawn smaller and higher — the 28×28 input low and large in the foreground, the verdict high, small and far off. Both ramps anchor on `_layerCentre()`, which the hand-tuned `PLACEMENT` already puts on one flat y ≈ 220 line for all six layers, so the ramp alone establishes the staircase and nothing needs renormalising. `RISE` is split either side of that line so the composition stays centred in its box; `SHRINK` is one-directional, since scaling the input *up* is the only direction that can overflow the reference frame. Everything routes through `_project()` — planes fold the scale into the cell size, so outline, cells and the plane-local line width shrink as one — which is why wires, captions and verdict follow the ramp for free. The caption strip deliberately stays **level** while the pipeline climbs away from it: it reads as the ground line the scene recedes over. The pointer then modulates this with **two** more effects. (1) **Yaw**: `SHEAR_X` is not a constant at draw time — the shear is re-derived each frame from the pointer's x, so the planes physically re-lean and the whole scene turns about its vertical axis. Every plane pivots about its **own vertical centre**, which is what makes this cheap: mid-height is invariant under the pivot and every anchor in the layout (wires, captions, the verdict) is taken at mid height, so the hand-tuned placement is pixel-identical at rest and the wires stay attached at full deflection. The neuron columns lean by the same angle (`_lean()`, shear delta ÷ `SQUASH` since their y is not foreshortened) — without that the conv half would turn while the dense half stayed frozen. The range is **asymmetric** (`YAW_UPRIGHT` 0.26 > `YAW_LEAN` 0.14): flatter planes only get more legible, steeper ones collapse a 28×28 digit into a diagonal ribbon. (2) **Translation**, depth-scaled per layer (`_depthFactor`, `LAYER_RECEDE`/`STACK_RECEDE`): the share is **signed**, +1 at the input through 0 mid-pipeline to −1 at the output, so the layers scissor about the scene's centre instead of drifting sideways together. A one-directional taper (the original 1 → 0.3) spends nearly all its travel budget sliding the whole scene and leaves only a few px of the differential the eye actually reads as depth — which is why `PAR_X`/`PAR_Y` can be this small and still register. `PAR_X` is capped by the left-edge mask, not by taste: the input plane sits at the scene's left edge and more travel pushes the most legible thing in the picture into the gradient. That is the reason the hero is hand-rolled rather than built on TensorSpace.js (Three.js + TensorFlow.js, several hundred KB, unmaintained). Training is deliberately a separate step from the generator, so the generator stays deterministic, network-free and CI-runnable; `test/cnn-activations.test.mjs` regenerates and fails on drift.
