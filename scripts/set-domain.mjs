...
/* Everything below runs inside main(), and main() runs only when this file is
   the process entry point.

   It used to be 60 lines of top-level statements, which meant `import
   './set-domain.mjs'` rewrote the domain across every HTML file, robots.txt,
   sitemap.xml, scripts/lib/site.json and public/CNAME as a side effect of the
   import. Nothing imports it today — but two generators in this directory had
   exactly that shape, and it made both their test suites unfalsifiable
   (see the note in scripts/generate-analytics.mjs). This one would have been
   worse than a blind test: it rewrites the site's origin, and the first
   argument it reads is process.argv[2], which under a test runner is whatever
   flag happened to be there.

   Tests read; generators write, and only when asked. */
function main() {
