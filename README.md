...
│   ├── ui.js                  Chrome/UI — navbar, mobile menu, command palette, counters, toast, back-to-top
│   ├── three-context.js       Shared THREE binding + test mocking hook
│   ├── utils.js               isLowPowerDevice, prefersReducedMotion, hasWebGLSupport, getTopoJSON, hasCoords
...
│   ├── globe.js               Globe3D + GlobeFallback2D
...
│   ├── land-50m.json          TopoJSON world land (50m) — build-time source only, never deployed
│   ├── europe-land.json       Generated — the Europe coastline polylines the 2D map fetches (run generate-europe-land)
...
│   ├── generate-europe-land.test.mjs  Guards data/europe-land.json against drift + the sub-pixel simplify budget
│   ├── theme-rebuild.test.mjs      Guards the fresh-canvas rebuild after a theme switch
│   ├── globe.test.html             Interactive globe visualisation harness (manual, opened in a browser)
├── .cache/
...
```bash
npm test                        # run all tests (a glob over test/*.test.{js,mjs} —
                                #   a new test file is picked up with no edit here)
...
# europe-map.test.mjs, theme-rebuild.test.mjs and generate-europe-land.test.mjs
# run under `npm test` but have no dedicated shorthand
```
...
### `generate-europe-land` — extract the Europe coastline the 2D map draws

Reads `data/land-50m.json` (world coastlines, 545 KB, **not** deployed) and writes `data/europe-land.json` (~95 KB, deployed): the coastline polylines the travel page's 2-D map actually strokes. Three reductions, each one a decision the browser used to make on every visit — keep the rings that touch the Europe bounding box, split them into the subpaths the renderer would emit, then Douglas-Peucker at a sub-pixel epsilon. Bounds come from `EUROPE_BOUNDS` in `js/europe-map.js`, so there is no second copy to keep in step.

Re-run after changing `data/land-50m.json` or `EUROPE_BOUNDS`. `test/generate-europe-land.test.mjs` fails on drift.

```bash
npm run generate-europe-land
# or:
node scripts/generate-europe-land.mjs --dry-run
```
...
Switching the whole site to another palette = change the `active:` key in `data/palettes.yaml` (or add a new palette), then run **all three** of `npm run generate-theme`, `npm run generate-theme-toggle` and `npm run generate-favicons`. The middle one is easy to forget and used to be missing from this line: it owns `aria-pressed` on the navbar palette dots, so skipping it leaves all 21 pages marking the *previous* palette as selected. `test/theme-sync.test.js` now fails on exactly that.
...
After rotating, run `npm run generate-theme`, `npm run generate-theme-toggle` and `npm run generate-favicons` to propagate the colours — see the note under `generate-theme` for why the toggle step is not optional.
...
A Canvas2D flat map of Europe rendered from `data/europe-land.json` — the Europe-only coastline polylines that `npm run generate-europe-land` extracts from `data/land-50m.json` at build time. The map used to fetch the 545 KB world file and discard everything outside Europe in the browser; the generator does the same filtering (plus a sub-pixel simplify) once, so the page downloads 95 KB instead. Country outlines are drawn with the palette's coast colour on a dark background, matching the globe's visual style. Location pins from `data/locations.js` are plotted as pulsing rings or spikes using the same pin types as the 3D globe (`lived` / `work` / `travel`). Hovering a pin shows a tooltip with the location name and trip name (for trip waypoints). The map is loaded from `js/europe-map.js` on the travel page.
